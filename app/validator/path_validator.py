...
def validate_month_format(month: str):
    if re.match(r"^[0-1]\d$", month) == None:
        raise HTTPException(status_code=400, detail="Wrong month format on path. Correct format: MM (e.g. 05)")
...
def validate_day_format(day: str):
    if re.match(r"^[0-3]\d$", day) == None:
        raise HTTPException(status_code=400, detail="Wrong day format on path. Correct format: DD (e.g. 23)")
